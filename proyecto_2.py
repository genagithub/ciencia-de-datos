    ...
    html.Div(className="e2_container",children=[
        html.Div(id="data_1",className="e2_children",style={"color":"blue"},children=[   
            ...
            html.Ul(className="e2_ul",children=[
                html.Li(f"Producto: {product}",className="e2_li",style={"color":"blue"}),
                html.Li(f"Cantidad total vendida: {total_quantity_1}",className="e2_li",style={"color":"blue"}),
                html.Li(f"Ingresos totales: {total_incomes_1}$",className="e2_li",style={"color":"blue"})
            ])
        ]),
        html.Div(id="data_2",className="e2_children",children=[
            html.H2("Empleados",style={"color":"red","font-family":"sans-serif"}),
            html.Ul(className="e2_ul",children=[
                html.Li(f"Nombre: {name}",className="e2_li",style={"color":"red"}),
                html.Li(f"Cantidad total de productos: {total_quantity_2}",className="e2_li",style={"color":"red"}),
                html.Li(f"Ingresos totales: {total_incomes_2}$",className="e2_li",style={"color":"red"})
            ])
        ]),
        html.Div(id="data_3",className="e2_children",children=[
            ...
            html.Ul(className="e2_ul",style={"color":"green"},children=[
                html.Li(f"Órden: {order_id}",className="e2_li",style={"color":"green"}),
                html.Li(f"Cantidad total de productos: {total_quantity_3}",className="e2_li",style={"color":"green"}),
                html.Li(f"Ingresos totales: {total_incomes_3}$",className="e2_li",style={"color":"green"})
            ])
        ]) 
    ])
])
